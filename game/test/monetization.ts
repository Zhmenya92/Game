...

{
  ...
  // Списання «за фактом надісланого рану» тут БІЛЬШЕ НЕМАЄ: це й був
  // дефект 51 — гравець воскресав і просто не надсилав ран. Метод
  // прибрано разом із тестами на нього, бо мертвий код, що виглядає як
  // робочий шлях, — найпростіший спосіб повернути ту саму діру.
  ok('списання йде лише через оплату наперед', !('consume' in w));
  ok('дві оплати знімають дві одиниці',
    w.reserve(1) && w.reserve(1) && w.balance(1) === 1 && w.pendingFor(1) === 2);
